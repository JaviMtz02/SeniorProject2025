...
func _on_enter() -> void:
	var direction_to_burglar = (blue_guard.burglar.global_position - blue_guard.global_position).normalized()
	blue_guard.velocity = Vector2.ZERO
	blue_guard.move_and_slide()
	...
	attack_instance.global_position = blue_guard.global_position
	attack_instance.direction = direction_to_burglar
	
func _on_exit() -> void:
	blue_guard.burglar = null
