...
func _on_enter() -> void:
	var direction_to_burglar = (red_guard.burglar.global_position - red_guard.global_position).normalized()
	red_guard.velocity = Vector2.ZERO
	red_guard.move_and_slide()
	...
	attack_instance.global_position = red_guard.global_position
	attack_instance.direction = direction_to_burglar
	
func _on_exit() -> void:
	red_guard.burglar = null
