...
func _process(delta: float) -> void:
	if Input.is_mouse_button_pressed(MOUSE_BUTTON_LEFT) and mouse:
		...
		mouse = false
		var rows = int(mouse_copy.position.y / 160)
		var cols = int(mouse_copy.position.x / 160)
...
func swap_tiles(tile_src, tile_dist) -> void:
	...
	tiles[tile_src].global_position = tiles[tile_dist].position
	tiles[tile_dist].position = temp_pos
...
func shuffle_tiles() -> void:
	...
	for t in range(0,1000):
		...
		if tiles[tile] != $Slices/Tile16 and tile != previous and tile != previous_two:
			var rows = int(tiles[tile].position.y / 160)
			var cols = int(tiles[tile].position.x / 160)
			check_neighbor(rows, cols)
